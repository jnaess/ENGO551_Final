...
def can_submit_review():
    """False if one or more reviews from that user and isbn hav3e been submitted
    True if no previews reeviews from that user"""
    
    prev_user_reviews = db.execute("select count(*) from reviews \
                            where user_id = ?\
                            and isbn = ?", session.get("user_id"), session.get("isbn"))
    
    if prev_user_reviews[0]["count"] > 0:
        return False
    return True
...
def book():
    ...
    if request.method == "POST":
    ...
    elif request.method == "GET" and session.get("isbn"):
        #then a review may have been submitted and the session contains an isbn to search from
        
        #passed as session variable
        Book["isbn"] = session.get("isbn")
        ...
        if dic["totalItems"] == 0:
            ...
            Book["year"] = int(dic["items"][0]["volumeInfo"]["publishedDate"][0:4])
            Book["averageRatings"] = dic["items"][0]["volumeInfo"]["averageRating"]
            Book["title"] = dic["items"][0]["volumeInfo"]["title"]
            Book["ratingsCount"] = dic["items"][0]["volumeInfo"]["ratingsCount"]
            Book["author"] = dic["items"][0]["volumeInfo"]["authors"][0]
...
def review():    
    ...
    if request.method == "POST" and can_submit_review():
